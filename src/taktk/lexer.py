...
from pygments.token import Number  # Operator,
...
class TaktlLexer(RegexLexer):
    ...
    filenames = ["*.taktl"]
    aliases = [""]
    mimetypes = ["text/taktl"]
    url = "https://github.com/ken-morel/taktk"
    version_added = "0.1.0"
    ...
    tokens = {
        "root": [
            (
                r"\\([\w\._]+)(\:[\w_]+)?",
                bygroups(Punctuation, Name.Tag, Name.Other),
                "args",
            ),
            (r"(!)(enum)", bygroups(Punctuation, Keyword), "enum-content"),
        ...
        "args": [
            ...
            (
                r"([:\w_]*)(\:)([\w_]*)(\=)",
                bygroups(
                    Name.Attribute, Punctuation, Name.Attribute, Punctuation
                ),
                "literal",
            ),
            (
                r"([:\w_]*)(\:)([\w_]*)",
                bygroups(Name.Attribute, Punctuation, Name.Attribute),
            ),
        ...
        "literal": [
            (r"False|True|None|Nil", Keyword.Constant, "#pop"),
            (
                (
                    r"\b(?:ACTIVE|ALL|ANCHOR|ARC|BASELINE|BEVEL|BOTH|BOTTOM"
                    r"|BROWSE|BUTT|CASCADE|CENTER|CHAR|CHECKBUTTON|CHORD"
                    r"|COMMAND|CURRENT|DISABLED|DOTBOX|E|END|EW|EXTENDED|FALSE"
                    r"|FIRST|FLAT|GROOVE|HIDDEN|HORIZONTAL|INSERT|INSIDE|LAST"
                    r"|LEFT|MITER|MOVETO|MULTIPLE|N|NE|NO|NONE|NORMAL|NS|NSEW"
                    r"|NUMERIC|NW|OFF|ON|OUTSIDE|PAGES|PIESLICE|PROJECTING"
                    r"|RADIOBUTTON|RAISED|RIDGE|RIGHT|ROUND|S|SCROLL|SE|SEL"
                    r"|SEL_FIRST|SEL_LAST|SEPARATOR|SINGLE|SOLID|SUNKEN|SW|TOP"
                    r"|TRUE|UNDERLINE|UNITS|VERTICAL|W|WORD|X|Y|YES"
                    r"|__builtins__|__cached__|__doc__|__file__|__loader__"
                    r"|__name__|__package__|__spec__)\b"
                ),
                Keyword.Constant,
                "#pop",
            ),
            ...
            (r"@[\w.]+", bygroups(Punctuation, Name), "#pop"),
            (r"-?[\d,]+", Number, "#pop"),
            (r"\{\{", Punctuation, "binding"),
            (r"(\{)(\$)", bygroups(Punctuation, Keyword), "expression"),
...
# Note: `lambdas`, `expressions-common`, `qualified-name`, and
# `illegal-assignment-expressions` token states are placeholders
# and should be replaced with the actual rules or states defined elsewhere in
# your lexer if those features are needed.
